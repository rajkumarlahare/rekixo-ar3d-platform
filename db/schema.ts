...
export const plots = sqliteTable("plots", { projectId:text("project_id").notNull().default("tiyansh-prime-square").references(()=>projects.id), id:text("id").notNull(), sqft:real("sqft").notNull(), sqm:real("sqm").notNull(), sqyd:real("sqyd").notNull(), dimensions:text("dimensions").notNull(), road:text("road").notNull(), front:real("front"), depth:real("depth"), dimensionUnit:text("dimension_unit"), frontEdgeIndex:integer("front_edge_index"), depthEdgeIndex:integer("depth_edge_index"), frontLabel:text("front_label"), depthLabel:text("depth_label"), sideDimensions:text("side_dimensions"), polygon:text("polygon").notNull().default(""), status:text("status").notNull().default("available"), notes:text("notes").notNull().default(""), featured:integer("featured",{mode:"boolean"}).notNull().default(false), updatedAt:text("updated_at").notNull() },table=>({pk:primaryKey({columns:[table.projectId,table.id]})}));
