...
test("share builder adds a dedicated AR3D footer without cropping or covering the source", async () => {
  ...
  assert.ok(branding.includes('version: "2026-09-19-footer-v5"'));
  ...
  assert.ok(manager.includes("bitmap.width - sourceSampleWidth"));
  assert.ok(manager.includes("bitmap.height - sourceSampleHeight"));
  assert.ok(manager.includes("footerOutlierTrimRatio"));
  assert.ok(manager.includes("context.createLinearGradient"));
