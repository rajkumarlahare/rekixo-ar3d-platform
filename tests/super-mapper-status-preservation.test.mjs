...
test("mapper geometry upsert preserves live Booked/Sold and featured state on existing plots", () => {
  assert.match(
    source,
    /polygon=excluded\.polygon,inventory_active=1,notes=excluded\.notes,updated_at=excluded\.updated_at/,
  );
...
test("new mapper rows can still insert an initial status while conflict updates remain geometry-only", () => {
  assert.match(
    source,
    /INSERT INTO plots \(project_id,id,sqft,sqm,sqyd,dimensions,road,[^)]*polygon,status,notes,featured,inventory_active,updated_at\)/,
  );
